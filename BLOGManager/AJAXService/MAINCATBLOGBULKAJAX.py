...
from ..models import BlogMainCat
# ==============================================================================
# BUKL ACTIVATE MAIN CATEGORY USING AJAX
# ==============================================================================
@superadmin_required
def BULK_ACTIVATE_BLOG_MAIN_CATEGORY(request):
        ...
        if request.user.token == request.session['token']:
            ...
            if bulk_target:
                for target in bulk_target.split(","):
                    try:
                        obj = BlogMainCat.objects.get(pk=target)
                        obj.is_active = True
                        obj.save()
                    except Exception:
...
# ==============================================================================
@superadmin_required
def BULK_DEACTIVATE_BLOG_MAIN_CATEGORY(request):
    if request.session.has_key('SUPERADMIN') and request.session.has_key('ADMIN-PASS') and request.session.has_key('ADMIN-ROLE'):
        if request.user.token == request.session['token']:
            bulk_target = request.POST.get('id')
            print(bulk_target)
...
# ==============================================================================
@superadmin_required
def BULK_DELETE_BLOG_MAIN_CATEGORY(request):
    if request.session.has_key('SUPERADMIN') and request.session.has_key('ADMIN-PASS') and request.session.has_key('ADMIN-ROLE'):
        if request.user.token == request.session['token']:
            bulk_target = request.POST.get('id')
            if bulk_target:
