...
from ..utils import SETTINGResource
from datetime import datetime
from tablib import Dataset

@superadmin_required
def SETTINGImport(request):
            ...
            try:
                if request.method == 'POST':
                    ...
                    if file_format == 'CSV':
                        ...
                        SETTING.import_data(data, dry_run=False)   
                        AdminActivity.objects.create(admin=request.user, log=f'<span class="text-success"> [ CSV SETTINGS ] - Imported<span>')
                        ...
                        SETTING.import_data(getJSON, dry_run=False) 
                        AdminActivity.objects.create(admin=request.user, log=f'<span class="text-success"> [ JSON SETTINGS ] - Imported<span>')
        ...
        if request.user.is_superadmin and request.user.token == request.session['token']:
            ...
            if request.method == 'POST':
                ...
                if file_format == 'CSV':
                    ...
                    response['Content-Disposition'] = f'attachment; filename="CSV_SETTING_LIST-{datetime.now().strftime("%m/%d/%Y, %H:%M:%S")}.csv"'
                    AdminActivity.objects.create(admin=request.user, log=f'<span class="text-danger"> [ CSV SETTINGS ] - Exported<span>')
                    return response

                elif file_format == 'JSON':
                    response = HttpResponse(dataset.json, content_type='application/json')
                    response['Content-Disposition'] = f'attachment; filename="JSON_SETTING_LIST-{datetime.now().strftime("%m/%d/%Y, %H:%M:%S")}.json"'
                    AdminActivity.objects.create(admin=request.user, log=f'<span class="text-danger"> [ JSON SETTINGS ] - Exported<span>')
                    return response                
                elif file_format == 'XLS (Excel)':
                    response = HttpResponse(dataset.xls, content_type='application/ms-excel')
                    response['Content-Disposition'] = f'attachment; filename="EXCEL_SETTING_LIST-{datetime.now().strftime("%m/%d/%Y, %H:%M:%S")}.xls"'
                    AdminActivity.objects.create(admin=request.user, log=f'<span class="text-danger"> [ EXCEL SETTINGS ] - Exported<span>')
                    return response   
